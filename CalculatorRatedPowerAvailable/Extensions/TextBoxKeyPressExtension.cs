...
namespace CalculatorRatedPowerAvailable.Extensions
{
    public static class TextBoxKeyPressExtension
    {
        ...
        /// <param name="sender"></param>
        /// <returns></returns>
        public static bool TextBoxWithoutMinusHandle(this KeyPressEventArgs e, object sender)
        {
            if (!char.IsControl(e.KeyChar) && !char.IsDigit(e.KeyChar) &&
                (e.KeyChar != ','))
            {
            ...
            if (e.KeyChar == '0')
            {
                if (string.IsNullOrWhiteSpace((sender as TextBox).Text) && ((sender as TextBox).Text.IndexOf('0') > -1))
                    return true;
                if (!string.IsNullOrWhiteSpace((sender as TextBox).Text) && (sender as TextBox).Text.IndexOf('.') == -1)
                    return true;
            }
